...
test <- data.alpha %>%
  filter(Alpha.Metric == "Shannon") %>%
  # group_by(Diet) %>%
  rstatix::wilcox_test(data =., Alpha.Score ~ PrePostExp) %>%
  rstatix::adjust_pvalue(method = "BH") %>%
  rstatix::add_significance("p.adj")
...
```{r Path-Alpha-ExpDiets, fig.width = 5, fig.height = 2.5}
...
test <- data.da %>%
  group_by(Diet) %>%
  filter(Taxon == "Mycobacterium") %>%
  rstatix::wilcox_test(data =., value ~ PrePostExp) %>%
  rstatix::adjust_pvalue(method = "BH") %>%
  rstatix::add_significance("p.adj") %>%
  mutate(p.adj.signif = case_when(`p.adj.signif` == "*" ~ "*",
                                  `p.adj.signif` == "**" ~ "*",
                                  `p.adj.signif` == "***" ~ "*",
                                  `p.adj.signif` == "****" ~ "*",
                                  `p.adj.signif` == "ns" ~ "ns"))
...
Fig.3.e <- 
  ...
  # filter(Alpha.Metric == "Shannon") %>%
    ...
    facet_grid(. ~ Diet, scale = "free") +
    scale_fill_manual(values = pal.Set1[c(2,1,4)]) +
    scale_color_manual(values = pal.Set1[c(2,1,4)]) + 
